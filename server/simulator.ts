...
export class SimulationService {
  ...
  ingest() {
    const sequence = this.ingested % 4;
    const result = this.store.mutate((state) => {
      if (sequence === 0) {
        ...
        state.cases.unshift({
          ...
          kind: "payment",
          counterparty: `Demo Retail Partner ${this.ingested + 1}`,
          invoice: `SIM-${String(this.ingested + 1).padStart(4, "0")}`,
          amount: 1250000 + this.ingested * 10000,
          dueDate: day(5),
          summary:
      ...
      if (sequence === 1) {
        ...
        state.payments.push({
          ...
          amount,
          date: day(),
          reference: `SIM-RECEIPT-${this.ingested + 1}`,
          key: uid(),
          createdAt: now(),
        });
        target.status =
